...
SELECT TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE))/4
	FROM PROFESSOR
...
-- 오늘날짜 - (부임일자 + MONTH(오늘날짜 - 부임일자))

SELECT ENAME 
	 ,TRUNC(MONTHS_BETWEEN(SYSDATE, HDATE) / 12)
	 ,MOD(TRUNC(MONTHS_BETWEEN(SYSDATE, HDATE)), 12)
	 ,SYSDATE - ADD_MONTHS(HDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HDATE)))
	FROM EMP;
...
SELECT PNAME || ' 교수의 부임일은 ' || TRUNC(HIREDATE, 'YYYY')|| ' 년 '
	 || TRUNC(HIREDATE, 'MM') || ' 월 ' || TRUNC(HIREDATE, 'DD') || ' 일입니다.' 
	FROM PROFESSOR;
