...
ALTER TABLE STUDENT ADD CONSTRAINT PK_STUDENT_SNO PRIMARY KEY(SNO); -- PK로 설정하려면 무조건 중복값이 없어야 한다, FK는 PK나 UK가 설정되어야만 설정할 수 있다.
...
-- 1-3. UNIQUE KEY(UK): 중복값을 허용하지 않는 제약조건(여러개 지정 가능)
--                      PK가 중복값 불가, INDEX, NOT NULL 제약조건을 가지고 있는 반면
--                      UK는 중복값 저장만 불가만 가지고 있는 제약 조건이다. 그래서 NULL 값이 저장된다.
CREATE TABLE EMP_UK(
...
CREATE TABLE EMP_NOT_NULL (
    ...
    JOB VARCHAR2(10) CHECK(JOB = '개발') NOT NULL,
...
-- 기본적으로 DEFAULT 제약조건이 먼저 설정되야 하는 DBMS가 대부분이다.
