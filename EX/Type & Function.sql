...
--CONCAT 함수는 매개변수를 두 개만 받을 수 있어서 여러개의 데이터나 다른 표현을 넣고 싶을 떄는 ||와 조합하여 사용한다/
SELECT CONCAT(SNAME, ': '|| AVR) 
	 FROM STUDENT;	
...
-- LENGTH: 데이터의 길이를 리턴
-- LEMGTHB: 데이터의 길이를 BYTE 단위로 리턴
-- 오라클의 기본 문자셋은 AL32UTF8 => 영어를 제외한 다른 문자는 3BYTE씩 계
SELECT DNAME
	 , LENGTH (DNAME)
	 , LENGTHB (DNAME)
	 FROM DEPT;
...
-- 학생의 학생번호, 학생이름, 전공, 학년, 4.5만점으로 치환된 평점을 조회하는데 평점은 소수점 둘째자리까지 조회(ROUND 함수 사용)
SELECT SNO 
	 , SNAME 
	 , MAJOR 
	 , SYEAR 
...
-- TRUNC(데이터, 소수점 자리수): 지정된 소수점 자리수까지 버림하여 변환
...
-- 1-6. 날짜 함수
-- ROUND(날짜, 날짜 단위): 지정한 날짜 단위까지의 날짜를 빼오고 나머지 날짜는 초기화, 버려지는 날짜의 절반이상 지났으면 올림한다.
...
-- TRUNC(날짜, 날짜 단위): 지정한 날짜 단위까지의 날짜를 빼오고 나머지 날짜는 초기화, 버려지는 날짜는 그냥 초기화.
SELECT SYSDATE 
	 , TRUNC(SYSDATE, 'YYYY')
	 , TRUNC(SYSDATE, 'MM')
	 , TRUNC(SYSDATE, 'DD')
...
-- MONTHS_BETWEEN(날짜1, 날짜2) : 날짜1에서 날짜2를 뻰 일자 차이를 개월수로 치환해서 리턴
SELECT MONTHS_BETWEEN(TRUNC(SYSDATE, 'DD'), TO_DATE('20230523', 'YYYYMMDD'))
	FROM DUAL;

-- ADD MONTHS(날짜, 숫자): 날짜에서 숫자만큼의 개월수를 더한 날짜 리턴
...
SELECT TO_DATE('2024/05/24 15:18:00', 'YYYY')
...
-- 과목이 없는 교수의 과목정보는 배정받지 못함이라고 조회 
...
-- 1-9. DECODE(컬럼명이나 조회해온 데이터, 조건 값1,            /*JAVA SWITCH CASE문과 비슷함*/
--             컬럼의 데이터나 데이터가 조건 값1과 일치할 때 실행될 내용, 
--             컬럼의 데이터나 데이터가 조건 값1과 일치하지 않을 때 실행될 내용)
...
--        조건값 N,
--        컬럼의 데이터나 데이터가 조건 값1과 일치할 때 실행될 내용, 
--        컬럼의 데이터나 데이터가 조건 값1과 일치하지 않을 때 실행될 내용(위의 조건과 모두 일치하지 않는 경우))
SELECT ENO
	 , ENAME
	 , DNO
	 , DECODE(DNO, 
...
-- 업무가 개발이면 50% 인산,업무가 경영이면 30%인상, 지원이면 20%이상, 그 외 업무면 10%인상 된 인상급여 조회
SELECT E.ENO 
	 , E.ENAME 
	 , E.JOB 
	 , E.SAL 
...
-- CASE 절에 컬럼이나 데이러를 지정하지 않은 경우(WHEN 절에 비교문을 사용하여 조건을 지정할 수 있다.)
SELECT ENO
	 , ENAME
	 , COMM
	 , CASE 
