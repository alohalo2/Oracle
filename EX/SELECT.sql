-- 1. 기본 Select 구문
-- 과목테이블에서 과목번호, 과목이름, 학점, 담당교수번호 조회
SELECT CNO
	 , CNAME
	 , ST_NUM
	 , PNO
...
-- 2. 컬럼이나 테이블에 별칭 붙이기
-- 2-1. 컬럼에 별칭붙이기
-- 조회해올 데이터의 컬럼에 별칭을 붙이 수 있다. 조회된 데이터는 새로운 가상테이블을 생성하는데
-- 가상테이블의 컬럼면은 붙인 별칭으로 할당된다. 별칭은 한글로 붙여도 무방하나 대부분 영어로 붙인다.
...
-- 조회된 모든 컬럼의 데이터가 중복되어 중복으로 인식한다.
SELECT DISTINCT JOB
	 , MGR
	 FROM EMP;
...
SELECT SNO
	 ...
	 , AVR
	 FROM STUDENT
	 ORDER BY SYEAR DESC, AVR DESC; -- 1등부터 차례대로
...
-- WHERE 절은 FROM니아 JOIN-ON절 다음에 작성한다.
-- 7-1. WHERE 절에서 값의 크기를 비교할 수 없을 때는 자바와 마찬가지로 부등호를 사용한다.
-- 사원 중 급여가 3000이상인 사원번호, 사원이름, 급여 조회
SELECT ENO
	 , ENAME
	 , SAL
	 FROM EMP
...
-- 학생 중에 평점이 3.0 이하인 학생번호, 학생이름, 전공, 학년, 평점조회
SELECT SNO
	 , SNAME 
	 , MAJOR
	 , SYEAR 
...
-- 전공이 화학과인 학생의 학생번호, 학생이름, 전공, 학번 조회	
SELECT SNO
	 , SNAME 
	 , MAJOR
	 , SYEAR 
...
-- 전공이 화학과가 아닌 학생의 학생번호, 학생이름, 전공, 학번 조회
SELECT SNO
	 , SNAME 
	 , MAJOR
	 , SYEAR 
...
-- 급여칼럼에 있는 모든 데이터가 문자열로 변환되는 과정이 필요하다.
...
/*SELECT ENO
	 ...
	 , HDATE
	 FROM EMP
	 WHERE TO_CHAR(HDATE, 'YYYY-MM-DD:HH24:MI:SS') < '2000-10-10:23:59:59'; -- 컬럼의 DATATYPE을 바꾸면 안된다!*/
...
-- 부서번호가 10이거나 30인 사원 중 급여가 3000이상인 사원의 사원번호, 사원이름, 급여, 부서 번호 조회
SELECT ENO  
	 , ENAME 
	 , SAL 
	 , DNO  
...
-- 평점이 3.0이상 3.7이하이면서 학년이 2학년 이상 4학년 이하인 학생의 학생번호, 학생이름, 평점, 힉년, 전공 조회
SELECT SNO
	 , SNAME
	 , AVR
	 , SYEAR
...
SELECT PNO
	 ...
	 FROM PROFESSOR
	 WHERE HIREDATE
	 	BETWEEN TO_DATE('20000101 00:00:00', 'yyyyMMdd HH24:mi:ss') -- 대소문자 상관 없음
	 		AND TO_DATE('2001/12/31 23:59:59', 'yyyy/MM/dd HH24:mi/ss') 

-- 10. 여러개의 데이터와 값을 비교하는 IN 절
-- 전공이 화학이거나 물리인 학색의 학생번호, 학생이름, 전공, 학년 조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 큰 따옴표("")는 컬럼에 대한 별칭(alias)를 붙여줄때만 사용한다.
