...
-- 4. 레코드: 다양한 타입의 변수들을 모아놓은 자료형. (테이블의 한 행을 하나의 레코드라고 생각하면됨, 레코드 생성 = 한 행을 레코드로 만든다)
DECLARE
	-- 레코드의 선언
...
BEGIN
    LOOP
        ...
        STUROWARR(IDX).SYEAR := MOD(IDX, 4) + 0.33;
...
BEGIN
    ...
    -- EXISTS(인덱스): 지정한 인덱스에 데이터가 존재하는지 TRUE, FALSE로 리턴
    ...
    ELSE
        DBMS_OUTPUT.PUT_LINE('INDEX 3의 값이 없습니다. ');
    END IF;
...
-- 6-3. 커서 파라미터: 커러를 선언할 때 파라미터도 함께 선언해서 고정적인 쿼리의 결과가 아닌 (임시 저장공간,JAVA의 버퍼 개념)
...
DECLARE 
    ...
    STUREC STU_REC;
BEGIN
    -- 커서를 오픈할 떄 지정된 파라미터 데이터타입의 값들을 전달한다.
    OPEN STU_CUR(1, 2.7);
...
-- 파라미터로 전달된 부서번호가 같고 파라미터로 전달된 급여이상되는 급여를 받는 사원 목록을 조회
-- EMP 테이블의 모든 컬럼을 조회
...
BEGIN
    ...
    LOOP
        FETCH EMP_CUR INTO EMPREC.EMP_ROW;
    
        EXIT WHEN EMP_CUR%NOTFOUND;
    
        DBMS_OUTPUT.PUT_LINE('01부서에서 급여가 3000이상인 사원목록');
        DBMS_OUTPUT.PUT_LINE(EMPREC.EMP_ROW.ENO);
        DBMS_OUTPUT.PUT_LINE(EMPREC.EMP_ROW.ENAME);
        DBMS_OUTPUT.PUT_LINE(EMPREC.EMP_ROW.JOB);
        DBMS_OUTPUT.PUT_LINE(EMPREC.EMP_ROW.MGR);
        DBMS_OUTPUT.PUT_LINE(EMPREC.EMP_ROW.SAL);
        DBMS_OUTPUT.PUT_LINE(EMPREC.EMP_ROW.COMM);
        DBMS_OUTPUT.PUT_LINE(EMPREC.EMP_ROW.DNO);
        DBMS_OUTPUT.PUT_LINE('----------------------');
...
-- 6-4 목시적 커서: 쿼리를 실행하면 쿼리의 실행 결과를 오라클 내부에서 저장하고 있는데
--               오라클 내부에 저장된 쿼리의 결과를 확인할 수 있는 커서.
--                따로 커서를 선언하지 앟고 사용할 수 있는 커서.
--                SQL%NOTFOUND, SQL%FOUND, SQL%ROWCOUNT, SQL%ISOPEN 속성만 사용할 수 있다.
--                목시적 커서는 DML에서도 사용이 가능하다.
BEGIN
    UPDATE STUDENT_RECORD 
        SET 
            SYEAR = 5
    ...
    -- 목시적 커서를 이용해서 위 쿼리에 영향받은 행의 개수를 가져올 수 있다.
