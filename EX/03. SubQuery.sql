...
-- 공융의 일반화학 기말고사 성적보다 높은 학생의 학생번호, 학생이름, 과목번호, 기말고사 성적 조회
-- 공융의 일반화학 기말고사 성적
SELECT SST.SNO 
	 , SST.SNAME 
	 , SSC.CNO 
...
-- 학생번호, 학생이름, 과목번호, 과목이름, 기말고사 성적, 기말고사 성적 등급, 담당 교수번호, 담당 교수이름 조회하는데
...
-- 첫 번째 쿼리에서 컬럼의 개수, 데이터 타입이 결정되기 때문에 두 번째 쿼리는 첫 번째 퀄리의 컬럼의 개수, 데이터 타입을 따라야한다.
SELECT PNO
	 , PNAME
	 , HIREDATE
	 FROM PROFESSOR
	 WHERE HIREDATE >= TO_DATE('20000101', 'YYYYMMDD HH24:MI:SS')
UNION
SELECT ENO
	 , ENAME
	 , HDATE
	 FROM EMP
	 WHERE HDATE >= TO_DATE('20000101', 'YYYYMMDD HH24:MI:SS');

-- UNION은 중복을 제거해서 합집합 연산을 해준다.
-- 평점이 3.0 이상인 학생의 학생번호, 학생이름, 학년, 평점과 학년이 3학년의 학생번호, 학생이름, 학년, 평점을 함께 조회
-- 중복제거 있음
SELECT ST.SNO 
	 , ST.SNAME 
	 , ST.SYEAR 
...
-- 교집합 연산자를 물리, 화학, 학생중 평점이 3.0 이상인 학생의 학생번호, 학생이름, 전공, 평점 조회
SELECT SNO 
	 , SNAME 
	 , MAJOR 
	 , AVR
