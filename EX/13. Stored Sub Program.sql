...
IS
     CURSOR CUR_ST_SC_GRADE IS -- 테이블과 컬럼수랑 내용을 맞춰줘야 한다.
        SELECT ST.SNO
             , ST.SNAME
             , C.CNO
             , C.CNAME
...
BEGIN
    ...
    -- 위에서 지정한 함수 결과의 데이터 타입의 값을 RETURN 구문을 사용해서 RETRUN
...
IS
    DIFF_DATES NUMBER;
BEGIN
    IF DATES1 > DATES2 THEN DIFF_DATES := DATES1 - DATES2;
    ELSIF DATES2 > DATES1 THEN DIFF_DATES := DATES2 - DATES1;
    ELSIF DATES1 = DATES2 THEN DIFF_DATES := 0;
    END IF;
    RETURN DIFF_DATES;
END;
/

SELECT F_DIFF_DATES(SYSDATE, TO_DATE('2024/06/04', 'YYYY/MM/DD'))
    FROM DUAL;

-- 1-4. TRIGGER: 특정 테이블의 데이터의변경이 있을 때 지정한 PL/SQL 구문을 실행하는 SUB PROGRAM
-- BEFORE TRIGGER
...
BEGIN
    ...
    -- 데이터가 추가될 때
        ...
        -- 데이터가 변경될 때
        ELSIF UPDATING THEN
            RAISE_APPLICATION_ERROR(-20001,'최저 시급보다 낮은 급여로 데이터를 수정할 수 없습니다.');
        ELSE 
