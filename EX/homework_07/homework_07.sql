...
CREATE TABLE SCORE_CHK (
    ...
    CNO VARCHAR2(8),
    RESULT NUMBER(3, 0),
    CONSTRAINT CHK_SCORE_CHK_RES CHECK(RESULT BETWEEN 60 AND 90)
);
...
CREATE TABLE COURSE_CONTSRAINT(
    ...
    PNO VARCHAR2(8),
    CONSTRAINT PK_COURSE_CONTSRAINT_CNO_CANME PRIMARY KEY(CNO, CNAME),
...
DECLARE
    ...
    COURSEPROFESSOR COURSE_PROFESSOR;
BEGIN
    
    SELECT C.CNO, C.CNAME, P.PNO, P.PNAME 
    INTO COURSEPROFESSOR.COURSE_NO, COURSEPROFESSOR.COURSE_NAME, COURSEPROFESSOR.PROFESSOR_NO, COURSEPROFESSOR.PROFESSOR_NAME
        FROM COURSE C
        JOIN PROFESSOR P
          ON C.PNO = P.PNO
        WHERE C.CNAME = '유기화학';
    
    DBMS_OUTPUT.PUT_LINE(COURSEPROFESSOR.COURSE_NO);
    DBMS_OUTPUT.PUT_LINE(COURSEPROFESSOR.COURSE_NAME);
    DBMS_OUTPUT.PUT_LINE(COURSEPROFESSOR.PROFESSOR_NO);
    DBMS_OUTPUT.PUT_LINE(COURSEPROFESSOR.PROFESSOR_NAME);
END;
